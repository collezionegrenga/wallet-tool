...
# Importa la funzione di scan dal tuo scanner.py
...
# Cache per i risultati delle scansioni
scan_cache = {}
...
# Gestore delle scansioni in background
class ScanManager:
    def __init__(self):
        self.pending_scans = {}
...
# Inizializza il gestore delle scansioni
...
def scan():
    ...
    # Controlla se abbiamo già una scansione recente in cache
    ...
    # Registra e avvia scansione in background
    scan_manager.register_scan(scan_id, wallet_address)
    # Restituisci ID per polling
    return jsonify({
        "status": "pending",
        "scan_id": scan_id,
...
def api_scan(wallet):
    ...
    return jsonify({
        ...
        "nfts": [],  # TODO: integrare NFT se necessario
...
def api_close():
    ...
    # Crea la transazione per la chiusura e split 90/10
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
